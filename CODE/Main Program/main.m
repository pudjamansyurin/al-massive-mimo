...
% NOTE!!!
% This main parameter is set to be used for obtaining Figure(2). If you want get another
% Figure result, change the parameter first.

%============================ MAIN PARAMETER=============================
N = 300;                % Number of subcarrier
M = 50:50:300;          % Number of Tx antenna (in one BS) 
K = 30;                 % Number of Rx antenna (= number of UE)
...
SNR_dB = 0;             % List of SNR [dB] values 
                ...
                for Si = 1 : length(SNR_dB); 
                    ...
                    for Fi=1:FRM;
                        ...
                        for Ci=1:length(Code);
                            ...
                            for n = 1:N;
                                [SINR(:,:,n),Sig,I] = calcSINR(A(:,:,n), Hf(:,:,n), SNR_L, Ko, Pc, SNR_L);
                            end
...
% For running fig 1, please set SNR_dB in a range (-10:1:10), M = 100, K = 10
figure(1);
...
for Ei=1:length(CSI);
    for Chi=1:length(Channel);
        for Ci=1:length(Code);
            SEE = SE(Ei,Chi,Ki,Ci,:);
            SEF = reshape(SEE, [1, numel(SEE)]);
            plot(M, SEF, genMark(Chi, Ci, Ei));
            hold on;    
            lgd(i) = strcat(CSI(Ei), {', '}, Channel(Chi), {', '}, Code(Ci)); 
            i = i + 1;
        end    
...
for Ki = 1:length(K);
    ...
    SEF3 = reshape(SEE, [1, numel(SEE)]);
    plot(M, SEF3, genMark(1,Ki,1));
    hold on;
end
grid on;
...
for Mi = 1:length(M);
    ...
    SEF4 = reshape(SEE, [1, numel(SEE)]);
    plot(K, SEF4, genMark(1,Mi,1));
    hold on;
end
grid on;
